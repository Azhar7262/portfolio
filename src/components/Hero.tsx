          ...
          <div className="lg:col-span-7 space-y-6 text-left">
            <Reveal>
              <div className="flex flex-wrap items-center gap-3">
                ...
                <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full text-xs font-semibold glass text-cyan-300 border-cyan-400/20 shadow-sm">
              ...
              <div className="space-y-3">
                <h1 className={`text-4xl sm:text-5xl lg:text-[3.6rem] font-extrabold tracking-tight leading-[1.1] ${dark ? 'text-white' : 'text-slate-900'}`}>
                  ...
                  <span className="relative inline-block text-gradient">
                    {PERSONAL_INFO.name}
                    <span className="absolute -bottom-1 left-0 w-full h-[3px] rounded-full bg-gradient-to-r from-cyan-400 via-blue-500 to-purple-500 opacity-80" />
                  </span>
                </h1>
                ...
                <div className="h-10 flex items-center text-xl sm:text-2xl lg:text-3xl font-bold font-mono">
                  ...
                  <span className={dark ? 'text-cyan-300' : 'text-cyan-700'}>{displayedText}</span>
            ...
            <Reveal delay={140}>
              <p className={`text-sm sm:text-base font-medium ${dark ? 'text-cyan-200/90' : 'text-cyan-900'}`}>
                {PERSONAL_INFO.title}
              </p>
            </Reveal>
              ...
              <div className="pt-3 flex flex-wrap items-center gap-3 sm:gap-4">
                ...
                <a
                  ...
                  className={`group flex items-center gap-2 px-6 py-3 rounded-2xl text-sm font-semibold glass-sheen transition-all duration-300 hover:-translate-y-1 ${
                    dark
                      ? 'glass text-slate-200 hover:text-cyan-300 border-cyan-400/30 hover:border-cyan-400/60'
                      : 'glass-light text-slate-700 hover:text-cyan-700 border-cyan-600/30'
                  }`}
                >
                  <Rocket className="w-4 h-4 text-cyan-400 group-hover:rotate-12 group-hover:-translate-y-0.5 transition-transform" />
                ...
                <a
                  ...
                  className="group flex items-center gap-2 px-6 py-3 rounded-2xl text-sm font-semibold text-slate-950 bg-gradient-to-r from-cyan-300 to-cyan-400 hover:from-cyan-200 hover:to-cyan-300 shadow-lg shadow-cyan-400/30 hover:shadow-cyan-300/50 hover:-translate-y-1 transition-all duration-300"
                >
          ...
          <div className="lg:col-span-5 relative">
            <Reveal delay={200}>
              ...
              >
                ...
                <div className="absolute -inset-4 rounded-[2rem] bg-gradient-to-tr from-cyan-500/25 via-blue-600/20 to-purple-600/25 blur-2xl animate-pulse-glow pointer-events-none" />

                {/* Liquid Glass Card */}
                <div className="relative p-px rounded-3xl bg-gradient-to-b from-cyan-400/40 via-blue-500/20 to-purple-500/30">
                  <div className={`p-6 sm:p-8 rounded-3xl glass-sheen ${dark ? 'glass-deep' : 'glass-deep-light'} animate-float-soft`}>
                    ...
                    <div className={`flex items-center justify-between pb-6 border-b ${dark ? 'border-white/10' : 'border-slate-900/10'}`}>
                      <div className="flex items-center gap-3">
                        <div className="w-12 h-12 rounded-2xl bg-gradient-to-tr from-cyan-400 via-blue-600 to-indigo-600 p-px shadow-lg shadow-cyan-500/40">
                          <div className={`w-full h-full rounded-2xl flex items-center justify-center text-cyan-300 font-bold ${dark ? 'bg-slate-950' : 'bg-white'}`}>
                            AWS
                          </div>
                        </div>
                        <div>
                      ...
                      <span className="px-2.5 py-1 rounded-full text-[10px] font-bold tracking-wider uppercase bg-cyan-500/15 text-cyan-300 border border-cyan-400/30">
                        Verified
                      </span>
                    </div>
                        ...
                        {[
                          ...
                          <span
                            ...
                            className={`px-3 py-1.5 rounded-lg text-xs font-medium border transition-all duration-300 hover:-translate-y-0.5 ${
                              dark
                                ? 'bg-white/5 border-white/10 text-cyan-300 hover:border-cyan-400/50 hover:bg-cyan-500/10'
                                : 'bg-slate-900/5 border-slate-900/10 text-slate-700 hover:border-cyan-600/50 hover:bg-cyan-500/5'
                            }`}
                          >
                            {skill}
                          </span>
                ...
                {/* Floating badges */}
                <div className="absolute -top-5 -right-4 px-4 py-2 rounded-2xl bg-gradient-to-r from-blue-600 to-indigo-600 text-white text-xs font-bold shadow-xl shadow-blue-500/40 flex items-center gap-2 animate-float" style={{ animationDelay: '0.5s' }}>
                  <ShieldCheck className="w-4 h-4 text-cyan-300" />
                  <span>AWS Certified</span>
                </div>

                <div className={`absolute -bottom-5 -left-4 px-4 py-2.5 rounded-2xl text-xs font-semibold shadow-xl flex items-center gap-2 animate-float ${dark ? 'glass-deep text-cyan-300 border border-cyan-400/40' : 'glass-deep-light text-cyan-700 border border-cyan-500/40'}`} style={{ animationDelay: '1.2s' }}>
            ...
            <Reveal key={idx} delay={idx * 90}>
              <div className={`p-5 rounded-2xl glass-sheen glow-hover h-full ${dark ? 'glass' : 'glass-light'}`}>
                <div className="flex items-center justify-between mb-2">
                  <div className={`p-2 rounded-xl border ${dark ? 'bg-cyan-500/10 border-cyan-400/20' : 'bg-cyan-50 border-cyan-200'}`}>
