              ...
              <div className={`group rounded-3xl glass-sheen glow-hover overflow-hidden flex flex-col justify-between h-full ${dark ? 'glass' : 'glass-light'}`}>
                ...
                <div className="relative h-56 overflow-hidden">
                  ...
                  <div className="absolute top-4 left-4 right-4 flex items-center justify-between">
                    <span className="px-3 py-1 rounded-full text-[11px] font-bold uppercase tracking-wider bg-amber-400 text-slate-950 shadow-lg shadow-amber-500/40">
                      {project.category}
                    </span>
                    <span className={`px-3 py-1 rounded-full text-[11px] font-mono font-medium backdrop-blur-md border ${dark ? 'bg-slate-950/70 text-amber-300 border-amber-400/20' : 'bg-white/80 text-amber-700 border-amber-200'}`}>
                      {project.date}
                    </span>
                  </div>
                  ...
                  <button
                    ...
                    className="absolute bottom-4 right-4 px-3.5 py-2 rounded-xl text-xs font-bold text-white bg-slate-950/80 border border-amber-400/40 backdrop-blur-md flex items-center gap-1.5 opacity-90 group-hover:opacity-100 hover:border-amber-300 hover:shadow-lg hover:shadow-amber-500/30 transition-all"
                  >
                ...
                <div className="p-6 space-y-4 flex-1 flex flex-col justify-between">
                  <div className="space-y-2">
                    <h3 className={`text-xl font-extrabold tracking-tight transition-colors group-hover:text-amber-300 ${dark ? 'text-white' : 'text-slate-900'}`}>
                      {project.title}
                    </h3>
                  ...
                  <div className="pt-2">
                    <div className="flex flex-wrap gap-1.5">
                      {project.technologies.map((tech, tIdx) => (
                        <span
                          key={tIdx}
                          className={`px-2.5 py-1 rounded-lg text-xs font-mono font-medium border ${dark ? 'bg-white/5 text-amber-300 border-white/10' : 'bg-amber-50 text-amber-700 border-amber-200'}`}
                        >
                          {tech}
                        </span>
                      ))}
                  ...
                  <div className={`pt-4 border-t flex items-center gap-3 ${dark ? 'border-white/10' : 'border-slate-900/10'}`}>
                    {project.githubUrl && (
                      <a
                        ...
                        className={`flex-1 flex items-center justify-center gap-2 py-2.5 px-4 rounded-xl text-xs font-bold border transition-all hover:-translate-y-0.5 ${
                          dark
                            ? 'border-white/10 bg-white/5 text-slate-200 hover:border-amber-400/60 hover:text-amber-300'
                            : 'border-slate-900/10 bg-white/70 text-slate-700 hover:border-amber-600/60 hover:text-amber-700'
                        }`}
                      >
                    ...
                    <button
                      onClick={() => onSelectProject(project)}
                      className="flex-1 flex items-center justify-center gap-2 py-2.5 px-4 rounded-xl text-xs font-bold text-white bg-gradient-to-r from-amber-500 to-orange-600 hover:from-amber-400 hover:to-orange-500 shadow-lg shadow-amber-500/30 hover:shadow-amber-400/50 hover:-translate-y-0.5 transition-all"
                    >
